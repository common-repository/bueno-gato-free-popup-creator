<?php
	...
	add_action('wp_head', 'pop_bgt_add_ajax_url_2');
	
	function pop_bgt_add_ajax_url_2()
	{ ?>
		<script> var ajax_url = '<?php echo admin_url('admin-ajax.php'); ?>';</script>
	<?php }
	
	function pop_bgt_big_foot()
	{
		$ucode = '<div style="margin: 2% auto; text-align: center; font-size: 1em;"><a href="">This awesome popup is created with WP Lead Plus.</a></div>';
		
		return $ucode;	
	}
	//create popup page, this is the UI where user creates and manage the popup
	function bgt_pop_sub_squeezers_popup_create_cb()
	{ ?>
	<div id="squeezer_popup">
		<div id="left_squeezer_popup" style="width: 20%; float: left;">	
			...
			<div style="position: fixed; top: 5%; right: 2%; font-size: 24px;"><a href="http://wpleadplus.com/?src=popadmin" target="_blank">Get more templates</a></div>
                    
			<?php bgt_pop_sq_common_editbox(); //start the editbox?>
	...
	function bgt_pop_show_popup_themes()
	{
	...
	//show the cta buttons
		add_action('wp_ajax_bgt_pop_show_pop_buttons', 'bgt_pop_show_pop_buttons_callback');
		

	function bgt_pop_show_pop_buttons_callback()
	{
		...
		$button_table = $wpdb->get_blog_prefix()."bgt_pop_cta_buttons";
	...
	add_action('wp_ajax_bgt_pop_popup_theme_loader', 'bgt_pop_popup_theme_loader_cb');
	
	function bgt_pop_popup_theme_loader_cb() {
		if (function_exists("curl_init")) //if curl available, use it to avoid complication
		{
			$ch = curl_init();
			curl_setopt($ch, CURLOPT_URL, trim(base64_decode($_POST['url'])).'/1/index.html');  
		...
		{
			$content = file_get_contents(trim(base64_decode($_POST['url'])).'/1/index.html');
		}		
		...
		$content = str_replace("assets/", trim(base64_decode($_POST['url'])).'/1/assets/', $content);
		
		echo "123dddsacxz".base64_encode($content)."123dddsacxz";
		die();
		}
	
	/* PARSE THE EMAIL AND SEND BACK TO THE CLIENT */
	add_action('wp_ajax_bgt_pop_popup_parse_autoresponder', 'bgt_pop_parse_autoresponder_callback');
	
	/* END PARSING THE EMAIL AND SEND BACK TO THE CLIENT */
	
		/* SHOW THE BUTTONS TO USERS */
		add_action('wp_ajax_bgt_pop_popup_show_buttons', 'bgt_pop_popup_show_button_cb');
	
		function bgt_pop_popup_show_button_cb()
		{
			//get the path to current theme button
		...
		//save the code to db
		add_action('wp_ajax_bgt_pop_popup_save_to_db', 'bgt_pop_popup_save_to_db_cb');
		
		function bgt_pop_popup_save_to_db_cb() {
			...
			try {
				...
				//global the wpdb
				global $wpdb;
				$popup_table = $wpdb->get_blog_prefix().'bgt_pop_sq_popup_code';
		...
		function bgt_pop_sub_squeezers_popup_manage_cb()
		{
			...
			<div id="popup_manage_container">
				...
				<div id="popup_manage_first_row" class="popup_manage_row" >
					...
					<h4>Your popups</h4>
					<?php bgt_pop_popup_manage_show_pop();?>					
					</div>
			...
			<h4>Current Options</h4>
				...
				global $wpdb;
				$table = $wpdb->get_blog_prefix().'bgt_pop_sq_popup_option';
		...
		function bgt_pop_popup_manage_show_pop()
		{
		...
		add_action('wp_ajax_bgt_pop_popup_manage_show_preview', 'bgt_pop_popup_manage_show_preview_cb');
		
		function bgt_pop_popup_manage_show_preview_cb()
		{
			$popup_id = $_POST['popup_id'];
		...
		add_action('wp_ajax_bgt_pop_popup_save_option', 'bgt_pop_popup_save_option_cb');
		
		function bgt_pop_popup_save_option_cb()
		{
		...
		add_action('wp_ajax_bgt_pop_popup_save_listing_option', 'bgt_pop_popup_save_listing_option_cb');
		
		function bgt_pop_popup_save_listing_option_cb()
		{
			global $wpdb;
		...
		add_action('wp_footer', 'bgt_pop_sq_popup_display');
		
		function bgt_pop_sq_popup_display()
		{

			$ucode = pop_bgt_big_foot();
			...
			} else 
			{
				...
				if ($frequency == 'once')
				{
					...
					var data = {action: "bgt_pop_pop_disable_pop", disable: "true"};
					var url = jQuery.trim("'.admin_url("admin-ajax.php").'");

					jQuery.post( ajax_url, data, function(response){
					jQuery("#sq_pop_outer").attr("id", "sq_pop_outer2");
...
//if background cover is on, the content on the website is covered. If it's off, content is not covered		
				if ($background_cover == 'pop_cover_no')
				{
					...
					} else if ($appear_position == 'pop_center')
					{
						...
						$inner_div = 'margin-top: 5%; position: relative;';
				...
				$return_code = "<div style='display: none; $outer_div z-index: 999999; $outer_background;' id='sq_pop_outer'><div style='$inner_div'>".base64_decode($popup_code).$ucode."</div></div><script>jQuery(document).ready(function(){".$display_script . $close_script."jQuery('head').append('".base64_decode($css_url)."');}); </script>";
				echo $return_code;
			}
		}
		
		//disable the popup when the user click the close btn
		add_action('wp_ajax_nopriv_bgt_pop_pop_disable_pop', 'bgt_pop_pop_disable_pop_cb');
		add_action('wp_ajax_bgt_pop_pop_disable_pop', 'bgt_pop_pop_disable_pop_cb');
		
		function bgt_pop_pop_disable_pop_cb()
		{
		...
		add_action('wp_ajax_bgt_pop_pop_delete_pop', 'bgt_pop_pop_delete_pop_cb');
		function bgt_pop_pop_delete_pop_cb()
		{
			$popup_id = $_POST['popup_id'];
			//perform the delete
			global $wpdb;
			$table = $wpdb->get_blog_prefix().'bgt_pop_sq_popup_code';
			$popup_table = $wpdb->get_blog_prefix().'bgt_pop_sq_popup_option';
		...
		add_action('wp_ajax_bgt_pop_pop_delete_pop_option', 'bgt_pop_pop_delete_pop_option_cb');
		function bgt_pop_pop_delete_pop_option_cb()
		{
			$op_id = $_POST['op_id'];
			//perform the delete
			global $wpdb;
 		...
 		//get popup shortcode
		add_shortcode('sq_pop_shortcode', 'bgt_pop_pop_gen_shortcode');
		function bgt_pop_pop_gen_shortcode($atts)
		{
			$ucode = pop_bgt_big_foot();
			extract($atts); 
			...
			$return_code = "<div style='display: none; $outer_div z-index: 999999; $outer_background' id='sq_pop_outer'><div style='$inner_div'>".base64_decode($popup_code).$ucode."</div></div><script>$display_script $close_script jQuery('head').append('$css_url');</script>";
			return $return_code;
		}